...
def load_dataset(folder_path, duration_col_name, label="Dataset"):
    ...
    if duration_col_name not in df.columns: return None, None
...
def run_experiment(train_df, test_df, features, title, ax, model_name=None):
    ...
    sns.regplot(x=y_test[idx], y=y_pred[idx], scatter=False, ax=ax, color='red', label=f'rho={rho:.2f}')
...
def compare_all():
    ...
    run_experiment(motr_train, motr_test, feats, "1. Train MoTR -> Test MoTR", axes[0, 0], model_name="rf_motr_to_motr_expms_spearman")
    run_experiment(et_train, et_test, feats, "2. Train ET -> Test ET", axes[1, 1], model_name="rf_et_to_et_expms_spearman")
    run_experiment(df_motr, df_et, feats, "3. Train MoTR -> Test ET (Cross-Modal)", axes[0, 1], model_name="rf_motr_to_et_cross_expms_spearman")
    run_experiment(df_et, df_motr, feats, "4. Train ET -> Test MoTR (Cross-Modal)", axes[1, 0], model_name="rf_et_to_motr_cross_expms_spearman")

    plt.suptitle("Universal Validation in EXP(-MS) w/ Spearman", fontsize=16)
    plt.savefig("comparison_matrix_expms_spearman.svg", format='svg')
