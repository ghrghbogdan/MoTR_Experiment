...
MODEL_SAVE_DIR = '/home/ghr/Documents/Bogdan/MoTR/REG/saved_models/'
...
def compute_linguistic_features(df, text_col='word'):
    ...
    word_feats = {w: [get_zipf(w), get_len(w), get_syl(w)] for w in unique_words}
...
def load_and_prep_dataset(folder, dur_col, label):
    ...
    df['ms'] = pd.to_numeric(df[dur_col], errors='coerce')
...
def run_experiment(X_train, y_train, X_test, y_test, title, ax, model_name=None):
    ...
    ax.set_title(f"{title}\nrho={rho:.2f} | MAE={mae:.4f}")
    ax.set_xlabel("Real exp(-ms)")
    ax.set_ylabel("Predicted exp(-ms)")
    ax.legend()
    ax.grid(True, alpha=0.3)

def compare_fusion_all():
    X_motr, y_motr, _ = load_and_prep_dataset(PATH_MOTR_FOLDER, COL_MOTR_DUR, "MoTR")
    try: X_et, y_et, _ = load_and_prep_dataset(PATH_ET_FOLDER, COL_ET_DUR, "Eye-Tracking")
    except: X_et = None
    ...
    run_experiment(X_motr_train, y_motr_train, X_motr_test, y_motr_test, "1. MoTR -> MoTR (Fusion Exp MS)", axes[0,0], model_name="rf_fusion_motr_to_motr_expms_spearman")
    run_experiment(X_et_train, y_et_train, X_et_test, y_et_test, "2. ET -> ET (Fusion Exp MS)", axes[1,1], model_name="rf_fusion_et_to_et_expms_spearman")
    run_experiment(X_motr, y_motr, X_et, y_et, "3. MoTR -> ET (Cross Exp MS)", axes[0,1], model_name="rf_fusion_motr_to_et_cross_expms_spearman")
    run_experiment(X_et, y_et, X_motr, y_motr, "4. ET -> MoTR (Cross Exp MS)", axes[1,0], model_name="rf_fusion_et_to_motr_cross_expms_spearman")
    
    plt.suptitle("FUSION MODEL: Universal Validation in EXP(-MS) w/ Spearman", fontsize=16)
    plt.savefig("fusion_comparison_expms_spearman.svg", format='svg')
