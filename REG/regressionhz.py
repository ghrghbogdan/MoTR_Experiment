...
def load_dataset_hz(folder_path, duration_col_name, label="Dataset"):
    ...
    df['ms'] = pd.to_numeric(df[duration_col_name], errors='coerce')
    df = df[(df['ms'] > MIN_DURATION_MS) & (df['ms'] < MAX_DURATION_MS)].copy()

    sort_keys = [k for k in ['expr_id', 'participant_id', 'para_nr', 'sentence_id', 'word_nr'] if k in df.columns]
    if sort_keys:
    ...
    df['target'] = 1000.0 / df['ms']
    df['target'] = np.clip(df['target'], 0.1, MAX_HZ)
    return df, feat_cols
...
def run_experiment(train_df, test_df, features, title, ax, model_name=None):
    ...
    sns.scatterplot(x=y_test[idx], y=y_pred[idx], alpha=0.15, ax=ax, color='darkgreen')
    sns.regplot(x=y_test[idx], y=y_pred[idx], scatter=False, ax=ax, color='lime', label=f'r={r:.2f}')
    ax.plot([0, MAX_HZ], [0, MAX_HZ], 'k--', alpha=0.5, label='Ideal')
    
    ax.set_title(f"{title}\nrho={rho:.2f} | MAE={mae:.1f}Hz")
    ax.set_xlabel("Real Speed (Hz)")
    ax.set_ylabel("Predicted Speed (Hz)")
    ax.legend()
...
def compare_all_hz():
    df_motr, feats = load_dataset_hz(PATH_MOTR_FOLDER, COL_MOTR_DUR, "MoTR Data")
    if df_motr is None: return

    motr_train, motr_test = train_test_split(df_motr, test_size=0.2, random_state=42)

    df_et, _ = load_dataset_hz(PATH_ET_FOLDER, COL_ET_DUR, "Eye-Tracking Data")
    ...
    run_experiment(motr_train, motr_test, feats, "1. Train MoTR -> Test MoTR (Hz)", axes[0, 0],
                   model_name="rf_motr_to_motr_hz")
    run_experiment(et_train, et_test, feats, "2. Train ET -> Test ET (Hz)", axes[1, 1],
                   model_name="rf_et_to_et_hz")
    run_experiment(df_motr, df_et, feats, "3. Train MoTR -> Test ET (Cross-Hz)", axes[0, 1],
                   model_name="rf_motr_to_et_cross_hz")
    run_experiment(df_et, df_motr, feats, "4. Train ET -> Test MoTR (Cross-Hz)", axes[1, 0],
                   model_name="rf_et_to_motr_cross_hz")
    
    plt.suptitle("Universal Validation in HERTZ (Speed)", fontsize=16)
    plt.savefig("comparison_matrix_hz.svg", format='svg')
