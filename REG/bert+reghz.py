...
def load_and_prep_dataset_hz(folder, dur_col, label):
    ...
    y_ms = df['ms'].values
    y_hz = 1000.0 / y_ms
    y_hz = np.clip(y_hz, 0.1, MAX_HZ)
    return X_fusion, y_hz, df

def run_experiment_hz(X_train, y_train, X_test, y_test, title, ax, model_name=None):
    ...
    ax.plot([0, MAX_HZ], [0, MAX_HZ], 'k--', alpha=0.5)
    ax.set_title(f"{title}\nrho={rho:.2f} | MAE={mae:.1f} Hz")
    ax.set_xlabel("Real Speed (Hz)"); ax.set_ylabel("Predicted Speed (Hz)")
    ax.legend()
    ax.set_xlim(0, MAX_HZ); ax.set_ylim(0, MAX_HZ)

def compare_fusion_all_hz():
    X_motr, y_motr, _ = load_and_prep_dataset_hz(PATH_MOTR_FOLDER, COL_MOTR_DUR, "MoTR")

    try:
        X_et, y_et, _ = load_and_prep_dataset_hz(PATH_ET_FOLDER, COL_ET_DUR, "Eye-Tracking")
    except:
        X_et = None
    ...
    run_experiment_hz(X_motr_train, y_motr_train, X_motr_test, y_motr_test, "1. MoTR -> MoTR (Fusion Hz)", axes[0,0],
                      model_name="rf_fusion_motr_to_motr_hz")
    run_experiment_hz(X_et_train, y_et_train, X_et_test, y_et_test, "2. ET -> ET (Fusion Hz)", axes[1,1],
                      model_name="rf_fusion_et_to_et_hz")
    run_experiment_hz(X_motr, y_motr, X_et, y_et, "3. MoTR -> ET (Cross-Modal Hz)", axes[0,1],
                      model_name="rf_fusion_motr_to_et_cross_hz")
    run_experiment_hz(X_et, y_et, X_motr, y_motr, "4. ET -> MoTR (Cross-Modal Hz)", axes[1,0],
